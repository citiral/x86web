...
impl Emulator {
    ...
    pub fn load_and_link(&mut self, file: &js_sys::Uint8Array) -> u32 {
        let pc = self.memory.load_and_link(file);
        if pc > 0 {
            self.eip = pc;
        }
        pc
...
impl Emulator {
    ...
    pub fn get_code8(&self, index: u32) -> u8 {
        self.memory.get_at(self.eip + index)
    }
    ...
    fn execute(&mut self) {
        ...
        console_log!("EIP = 0x{:08X}, Code = 0x{:02X}", self.eip, code);
        ...
        match code {
            ...
            0x81 => {
                ...
                match modrm.reg {
                    ...
                    _ => console_log!("not implemented: 81 /{}", modrm.reg),
                }
            },
            0x83 => {
                let modrm = modrm::ModRM::parse32(self);
                match modrm.reg {
                    ...
                    _ => console_log!("not implemented: 83 /{}", modrm.reg),
                }
            },
            ...
            0xF3 => instructions::endbr32(self),
            0xF7 => {
                let modrm = modrm::ModRM::parse32(self);
                match modrm.reg {
                    3 => instructions::neg_rm32(self, &modrm),
                    _ => console_log!("not implemented: F7 /{}", modrm.reg),
                }
            },
            0xFF => {
                let modrm = modrm::ModRM::parse32(self);
                match modrm.reg {
                    ...
                    _ => console_log!("not implemented: FF /{}", modrm.reg),
                }
            },
            _ => {
    ...
    pub fn dump_registers(&self) {
        ...
        for i in 0..REGISTERS_COUNT {
            console_log!("{} = 0x{:08X}", REGISTER_NAMES[i], self.registers[i]);
        }
        console_log!("EIP = 0x{:08X}", self.eip);
    }
